...
/**
 * 
 * Santa Maria Record Writer
 * 
 * @author mauro.pelucchi@gmail.com
 * @project Hammer-Project - Santa Maria
 *
 */
public class SantaMariaRecordWriter extends RecordWriter<Text, BSONWritable> {

	private static final Log LOG = LogFactory.getLog(SantaMariaRecordWriter.class);
     ...
     */
    public SantaMariaRecordWriter(final DBCollection c, final TaskAttemptContext ctx) {
        collection = c;
        context = ctx;
        ...
        try {
            ...
            LOG.info("SANTA MARIA Writing to temporary file: " + outputPath.toString());
            outputStream = fs.create(outputPath, true);
        } catch (IOException e) {
            LOG.error("SANTA MARIA  Could not open temporary file for buffering Mongo output", e);
        }
    }
