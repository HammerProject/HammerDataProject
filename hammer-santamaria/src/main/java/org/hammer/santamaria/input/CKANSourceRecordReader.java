 ...
 *
 */
public class CKANSourceRecordReader extends BaseDataSourceRecordReader {

	private static final Log LOG = LogFactory.getLog(CKANSourceRecordReader.class);
	...
	public CKANSourceRecordReader(final DataSourceSplit split) {
		super(split);
	}
	...
	private void getPackageList() {
		...
		LOG.info(split.getUrl() + PACKAGE_LIST_COMMAND);
		GetMethod method = new GetMethod(split.getUrl() + PACKAGE_LIST_COMMAND);
		...
		try {
			...
			if (doc.containsKey("result")) {
				this.dataset = (ArrayList<String>) doc.get("result");
