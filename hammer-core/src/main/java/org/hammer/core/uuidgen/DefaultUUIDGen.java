...
public class DefaultUUIDGen implements UUIDGen {
	...
	private String nextUUID() {
		...
		final BigInteger current = BigInteger.valueOf(System.currentTimeMillis());

		// the number of milliseconds since 15 October 1582
		final BigInteger countMillis = current.subtract(COUNT_START);

		// the count of 100-nanosecond intervals since 00:00:00.00 15 October
		// 1582
		final BigInteger count = countMillis.multiply(BigInteger.valueOf(10000));

		String bitString = count.toString(2);
		if (bitString.length() < 60) {
			final int nbExtraZeros = 60 - bitString.length();
			String extraZeros = "";
		...
		final byte[] bits = bitString.getBytes();
