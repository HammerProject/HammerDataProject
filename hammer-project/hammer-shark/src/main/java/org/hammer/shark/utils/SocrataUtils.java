...
public class SocrataUtils {
	 ...
	 * @throws Exception
	 */
	public static long CountPackageList(SparkSession spark, String url, String id) throws Exception {
	 ...
	 * @throws Exception
	 */
	public static BasicBSONList GetDataSet(SparkSession spark, String id, String url, int offset, int limit)
			throws Exception {
		BasicBSONList pList = new BasicBSONList();
		String socrataQuery = CreateWhereCondition(spark, id);

		String urlStr = url + "?$offset=" + offset + "&$limit=" + limit; // + "&$where=" + EncodeURIComponent(socrataQuery);
		SaveData(spark, id, urlStr, socrataQuery);
	 ...
	 * @throws Exception
	 */
	public static BasicBSONList GetDataSet(SparkSession spark, String id, String url)
			throws Exception {
		BasicBSONList pList = new BasicBSONList();
		LOG.info("SELECT RECORD BY SOCRATA -> ");
	 ...
	 * @return
	 */
	public static boolean CheckColumn(SparkSession spark, String id, String column) {
	...
	public static String CreateWhereCondition(SparkSession spark, String id) {

		String query = spark.sparkContext().conf().get("query-string");
		...
		try {
			...
			while (c < q.getQueryCondition().size()) {
				...
				boolean check = CheckColumn(spark, id, en.getName());
				if (check) {
	 ...
	 * @param count
	 */
	private static void SaveData(SparkSession spark, String datasource, String link, String searchQuery) {
		MongoClient mongo = null;
		...
		try {
			...
			StatUtils.SaveStat(statObj, spark.sparkContext().conf().get("list-result"), spark.sparkContext().conf().get("stat-result"));
	 ...
	 * @throws Exception
	 */
	public static String GetUrl(SparkSession spark, String id, String url, int offset, int limit, String socrataQuery)
			throws Exception {
